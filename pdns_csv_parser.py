...
filter_unwanted_rrtype = blacklist_record_types
sensor_name = ''
records = {}
cl_records = {}


class MParentDomain(object):
...
def process_record(ans):
  ...
  if len(ans) == 7:
    serv, date, domain, undef, rr, ttl, value = ans
  elif len(ans) == 8:
    serv, date, domain, undef, rr, ttl, pref, value = ans

    if rr == 'mx':
      value = ','.join([pref, value])
    else:
      return
  elif len(ans) == 13:
    serv, date, domain, undef, rr, ttl, primary_ns, contact, serial, slave_refresh, slave_retry, slave_expire, negative_ttl = ans

    if rr == 'soa':
      value = ','.join([primary_ns, contact, serial, slave_refresh, slave_retry, slave_expire, negative_ttl])
    else:
      return
  else:
    return
  ...
  if not parent in cl_records:
    cl_records[parent] = MParentDomain(parent)
...
def csvdump(recs, sep='|', print_title=True):
  if print_title:
    print 'domain{0}server{0}rr{0}ttl{0}value{0}firstseen{0}lastseen{0}matches'.format(sep)
...
if __name__ == '__main__':
  ...
  for l in iter(inputfile.readline, ''):
    ...
    # 192.55.83.30,1358631000.020359,ns171.hostgator.com,in,a,172800,184.173.195.204
    res = l.rstrip().split(',')
    res_len = len(res)
    ...
    if not res[4] in filter_rrtype or filter_rrtype[res[4]] in filter_unwanted_rrtype:
      '''
    ...
    if res[4] in ['a', 'aaaa', 'cname'] and filter_domains:
      filter_domains_found = False
      if res[2].endswith(ignore_domains) or res[6].endswith(ignore_domains):
